...
import { CircleX, ListEnd, ListPlus, Save, X } from 'lucide-react';
...
export default function FundDetail() 
{
  ...
  const fundData = location.state?.fundData; // Optional chaining prevents errors
  ...
  const handleAddEntry = () => {
    ...
    ? new Date(customDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' }).toLowerCase() 
    : today.toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' }).toLowerCase();
  ...
  return (
    <div className="min-h-screen bg-black text-gray-100 flex flex-col relative">
      ...
      <header className="fixed top-0 left-0 right-0 z-20 px-5 py-4 flex justify-between items-center bg-gray-950 ">
        ...
        <div className={`text-md sm:text-base text-gray-300 ${totalAmount < 0 ? 'bg-red-700' : 'bg-green-700'} px-3 py-1 rounded-lg`}>
          <span className="text-white font-semibold">₹ {totalAmount}</span>
        </div>
      </header>

      {/* Entries Table (Scrollable Area) */}
      <div className="flex-grow overflow-y-auto pb-25 mt-15  bg-gray-900">
